...
def register_callback_dashboard(app):
    ...
    def update_graph3_theme(toggle):
        ...
        # Obtenha os dados para o gráfico a partir do seu modelo
        lancamentos = Lancamento.objects.filter(categoria__tipo='2')  # Filtrar apenas as despesas
        gastos_por_mes = {month: 0 for month in range(1, 13)}  # Dicionário para armazenar os gastos por mês
        ...
        meses_com_valor = [calendar.month_name[month] for month, valor in gastos_por_mes.items() if valor > 0]
        gastos_com_valor = [valor for valor in gastos_por_mes.values() if valor > 0]
        ...
        trace = go.Bar(
            ...
            y=gastos_com_valor,
            name='Gastos',
            text=[locale.currency(valor, grouping=True) for valor in gastos_com_valor],  # Formatar como moeda
            textposition='auto'  # Exibir rótulo do dado
        )
    ...
    def calculate_total_receitas():
        receitas = Lancamento.objects.filter(categoria__tipo='1')  # Filtrar apenas as receitas
        total_receitas = sum(lancamento.valor for lancamento in receitas)
        return total_receitas
       
    def calculate_total_despesas():
        despesas = Lancamento.objects.filter(categoria__tipo='2')  # Filtrar apenas as despesas
